...
jnb-ping is a high-performance, non-blocking ICMP ping library for the JVM. Written in Kotlin, it uses the Java FFM API (`java.lang.foreign`) to call native POSIX APIs (`poll`, `sendto`, `recvfrom`) directly, enabling thousands of simultaneous pings without thread-per-connection overhead. Supports Linux (kernel >= 4.19.10) and macOS.
...
Tests require ICMP socket permissions. On Linux, set `sysctl net.ipv4.ping_group_range` (and `net.ipv6.ping_group_range` for IPv6). macOS works without special config. Some tests ping external hosts (8.8.8.8, youtube.com) so network access is required. Tests run with `local = True` (no sandbox), `timeout = "long"`, and `--enable-native-access=ALL-UNNAMED`.

CI runs on CircleCI with a self-hosted Linux machine executor (`brettwooldridge/ubuntu`).

Bazel 8.7.x, Kotlin 2.1 (language version), JVM 25. Bzlmod for dependency management (MODULE.bazel, no WORKSPACE).
...
- **`IcmpNative`** (`src/main/kotlin/com/zaxxer/ping/impl/IcmpNative.kt`) — FFM `MemoryLayout` definitions mirroring C structs (`struct icmp`, `struct ip`, `struct pollfd`, `struct sockaddr_in/in6`), `LibC` object with `Linker` downcall handles (errno via `Linker.Option.captureCallState`), byte-order helpers, ICMP checksum (RFC 1071). Platform-specific socket address layouts for BSD vs Linux (different `sockaddr` layouts, checksum responsibility, IP header presence).
...
- Off-heap `MemorySegment`s (GC-managed `Arena.ofAuto()`) for zero-copy native I/O; packet template pre-built once and copied per send.
